...
//
// 
// Related Topics 记忆化搜索 数组 动态规划 👍 31 👎 0

/**
 * 能转换到子问题，递推公式
 * <p>
 *              res[i][j] 直接卖出
 * f(i,j)=max   f(i-k,j)+f(k,j) (0<k<i)
 *              f(i,j-k)+f(i,k) (0<k<j)
 */
...
class Solution {
    public long sellingWood(int m, int n, int[][] prices) {
        ...
        for (int[] price : prices) {
            res[price[0]][price[1]] = price[2];
        }
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                long max=res[i][j];
                for (int k = 1; k <= i/2+1; k++) {
                    max=Math.max(dp[i-k][j]+dp[k][j],max);
                }
                for (int k = 1; k <= j/2+1; k++) {
                    max=Math.max(dp[i][j-k]+dp[i][k],max);
                }
                dp[i][j]=max;
            }
        }
