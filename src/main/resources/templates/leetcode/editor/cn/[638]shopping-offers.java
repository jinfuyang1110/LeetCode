...
// 0 <= special[i][j] <= 50 
// 
// Related Topics 位运算 记忆化搜索 数组 动态规划 回溯 状态压缩 👍 337 👎 0
...
class Solution {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        int min=10000;
        return d(price, special, needs);
    }
//记忆化搜索
    int  d(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        if (!map.containsKey(needs)) {
            ...
            for (List<Integer> list : special) {
                ...
                if (newNeeds.size() == needs.size()) {
                    cost=Math.min(cost,d(price,special,newNeeds)+list.get(size-1));
                }
            }
            map.put(needs,cost);
        }
